...
import com.hyd.ftpserver.utils.Consumer;
...
public class TextLineDecoder implements ProtocolDecoder {
    ...
    public TextLineDecoder(LineDelimiter delimiter) {
        this(new Charset[]{Charset.defaultCharset()}, delimiter);
    }
    ...
    public TextLineDecoder(Charset charset) {
        this(new Charset[]{charset}, LineDelimiter.AUTO);
    }

    public TextLineDecoder(Charset[] charsets) {
        this(charsets, LineDelimiter.AUTO);
    }
    ...
    public TextLineDecoder(Charset charset, String delimiter) {
        this(new Charset[]{charset}, new LineDelimiter(delimiter));
    }
     ...
     * and the specified <tt>delimiter</tt>.
     *
     * @param charsets The {@link Charset} to use
     * @param delimiter The line delimiter to use
     */
    public TextLineDecoder(Charset[] charsets, LineDelimiter delimiter) {
        if (charsets == null) {
        ...
        if (delimBuf == null) {
            ...
            tryCharsets(charset -> tmp.putString(delimiter.getValue(), charset.newEncoder()));
            tmp.flip();
            delimBuf = tmp;
        }
    }

    private void tryCharsets(Consumer<Charset> consumer) {
        boolean succeed;
        int index = 0;
        ...
        do {
            try {
                ...
                consumer.accept(charset);
                succeed = true;
            } catch (Exception e) {
                succeed = false;
            }
        } while (!succeed && index < charsets.length);
    }
            ...
            if (matched) {
                ...
                if (ctx.getOverflowPosition() == 0) {
                    ...
                    try {
                        ...
                        buf.get(data);
                        CharBuffer buffer = ctx.decode(ByteBuffer.wrap(data));
                ...
                if (matchCount == delimBuf.limit()) {
                    ...
                    if (ctx.getOverflowPosition() == 0) {
                        ...
                        try {
                            ...
                            writeText(session, ctx.decode(buf), out);
                        } finally {
                            buf.clear();
                        }
                    } else {
    ...
    private class Context {
        ...
        public CharBuffer decode(ByteBuffer wrap) {
            return charsets[0].decode(wrap);
        }

        public String decode(IoBuffer buf) throws CharacterCodingException {
            return buf.getString(charsets[0].newDecoder());
        }
    }
}